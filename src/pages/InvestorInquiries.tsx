...
import { Search, Eye, Trash2, Filter, MapPin } from 'lucide-react';
...
export default function InvestorInquiries() {
  ...
  const [filteredInquiries, setFilteredInquiries] = useState<InvestorInquiry[]>([]);
  ...
  const loadInquiries = async () => {
    try {
      setIsLoading(true);
      const response = await api.getInvestorInquiries();
      ...
      toast({
        title: 'Error',
        description: error.message || 'Failed to load investor inquiries',
        variant: 'destructive',
      });
      setInquiries([]);
    } finally {
  ...
  const calculateStats = (data: InvestorInquiry[]) => {
    const newStats = {
      total: data.length,
    ...
    if (search) {
      ...
      filtered = filtered.filter(
        (i) =>
          ...
          (i.companyName && i.companyName.toLowerCase().includes(searchLower))
      );
    }
  ...
  const handleUpdateInquiry = async () => {
    ...
    try {
      await api.updateInvestorInquiry(selectedInquiry._id, {
        status: updateStatus,
        notes: updateNotes,
      });
      ...
      toast({
        title: 'Success',
        description: 'Investor inquiry updated successfully',
      });
  ...
  const handleDeleteInquiry = async () => {
    ...
    try {
      await api.deleteInvestorInquiry(selectedInquiry._id);

      toast({
        title: 'Success',
        description: 'Investor inquiry deleted successfully',
      });
  ...
  return (
    <div className="p-6 space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold text-gray-900">Investor Inquiries</h1>
        <p className="text-gray-500 mt-1">
          Manage and track investor inquiries from potential partners
        </p>
      </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-gray-900">{selectedInquiry.city}</p>
                </div>
                <div>
                  <Label className="text-sm font-medium text-gray-700">State</Label>
                  <p className="mt-1 text-gray-900">{selectedInquiry.state}</p>
                </div>
                <div>
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Investor Inquiry</AlertDialogTitle>
            <AlertDialogDescription>
